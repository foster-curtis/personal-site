-- Test setup: runs first (pgTAP/`supabase test db` executes files in this directory in
-- alphabetical order, and "00000" sorts before every other file here). Everything above the
-- ROLLBACK-wrapped self-check at the bottom runs outside a transaction, so it persists in
-- the local database for every test file that runs after it in the same `supabase test db`
-- invocation.
--
-- Two things happen here:
--   1. Local-only grants so RLS is the thing under test, not a missing base privilege.
--   2. usebasejump/supabase-test-helpers, vendored (see below), for the auth-role-switching
--      boilerplate the tests in this directory rely on (tests.create_supabase_user,
--      tests.authenticate_as, etc).

create extension if not exists pgtap with schema extensions;

-- A fresh `supabase start` no longer auto-exposes new public-schema tables to the
-- anon/authenticated/service_role Data API roles (see the `auto_expose_new_tables` comment
-- in supabase/config.toml) -- so on a clean local stack, base table privileges are missing
-- entirely and every query fails with "permission denied" before Row Level Security is even
-- evaluated (confirmed directly: even service_role, which has BYPASSRLS, had zero grants on
-- a fresh `supabase start` here). Granting broadly is scoped to this local test database
-- only -- it is never applied to the hosted project -- and lets RLS policies be the actual
-- gate under test, matching how the app already behaves against the hosted project (which
-- predates the stricter default and already carries the equivalent grants).
...
-- (https://github.com/usebasejump/supabase-test-helpers/blob/main/supabase_test_helpers--0.0.6.sql),
-- MIT licensed. Vendored rather than installed via the dbdev package manager (the README's
-- recommended route) because dbdev requires the local Postgres container to reach
-- database.dev over the network at test-run time -- an external dependency this repo's
-- local/CI test runs shouldn't need. The only change from upstream is removing the
-- "\echo ... \quit" guard at the top of the original file, which exists to stop someone
-- from sourcing it directly instead of via `CREATE EXTENSION` -- exactly what this file
-- does on purpose.
-- ---------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------
-- End vendored supabase-test-helpers content.
-- ---------------------------------------------------------------------------------------

-- Self-check, wrapped and rolled back like every other test file in this directory: proves
-- the setup above actually worked, and gives pg_prove valid TAP output for this file.
BEGIN;
SELECT plan(2);
SELECT has_function('tests', 'authenticate_as', ARRAY['text'], 'tests.authenticate_as() should be installed');
SELECT ok(has_table_privilege('service_role', 'public.feedback_requests', 'INSERT'), 'service_role should have INSERT on public tables for this local test run');
SELECT * FROM finish();
ROLLBACK;
